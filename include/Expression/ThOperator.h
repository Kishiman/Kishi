...
template <typename Y, typename X> struct ThBaseOperator;
template <typename Y, typename X,
          template <typename, typename> class Base = ThBaseOperator>
struct OperatorRoot : Expression<Base<Y, X>> {
  ...
  struct Dyn : Base<Y, X> {
    ...
    virtual Y operator()(X x) const override { return func(x); }
...
struct OperatorRoot<Y, void, Base> : Expression<Base<Y, void>> {
  ...
  struct Dyn : Base<Y, void> {
    ...
    virtual Y operator()() const override { return func(); }
...
template <typename _Y, typename _X = void,
          template <typename, typename> class Base = ThBaseOperator>
struct ThOperator : OperatorRoot<_Y, _X, Base> {
  template <typename Y, typename X, typename Z>
  struct ThOperatorCascade
      : HBinaryExpression<Base<Y, X>, Base<X, Z>, Base<Y, Z>> {
  ...
  template <typename Y, typename X>
  struct ThOperatorCascade<Y, X, void>
      : HBinaryExpression<Base<Y, X>, BASEX, BASEY> {
    HBINARY(ThOperatorCascade, BASEYX, BASEX, BASEY)
    virtual Y operator()() const override { return (*base::a)((*base::b)()); };
  };
  template <typename Z> Ptr<Base<_Y, Z>> op(Base<_X, Z> &other) {
    return new ThOperatorCascade<_Y, _X, Z>(static_cast<Base<_Y, _X> &>(*this),
                                            const_cast<Base<_X, Z> &>(other));
  }
  template <typename Z> Ptr<Base<_Y, Z>> op(Ptr<Base<_X, Z>> other) {
    return op(*other);
  }
};
