...
template <typename S> S Zero() { return 0; }
...
#define EXPRESSION_OPP_END(CLASS)                                              \
  ...
  ;                                                                            \
  Ptr<CLASS> operator-() const { return new CLASS##Opp(SCTHIS(CLASS)); }
template <typename _Y, typename _X = void,
          template <typename, typename> class Base = ThBaseOperatorInt>
struct ThOperatorInt : ThOperator<_Y, _X, Base> {
  ...
  struct ThOperatorOpp : UnaryExpression<Base<Y, X>> {
    ...
    virtual Y operator()(X x) const override { return -(*base::express)(x); };
  };
  //****************************Specification*****************************
  template <typename Y>
  ...
  struct ThOperatorOpp<Y, void> : UnaryExpression<Base<Y, void>> {
    ...
    virtual Y operator()() const override { return -(*base::express)(); };
  };
  Ptr<_BASEYX> operator+(_BASEYX &other) const {
    return new ThOperatorAdd<_Y, _X>(SCTHIS(_BASEYX),
                                     static_cast<_BASEYX &>(other));
  }
  Ptr<_BASEYX> operator-(_BASEYX &other) const {
    return new ThOperatorSub<_Y, _X>(SCTHIS(_BASEYX),
                                     static_cast<_BASEYX &>(other));
  }
