...
template <typename _Tp> struct head_tail {
...
template <typename _Tp> using head_t = typename head_tail<_Tp>::head;
template <typename _Tp> using tail_t = typename head_tail<_Tp>::tail;

// extract args of template as tuple
template <typename _Tp> struct tuple_args {};
...
template <typename _Tp> using tuple_args_t = typename tuple_args<_Tp>::type;

template <typename _Tp, typename _tuple> struct assign_tuple_args {};
template <template <typename...> class _Template, typename... args,
          typename... _targs>
...
template <typename _Tp, typename _tuple> struct append_tuple_args_last {};
template <template <typename...> class _Template, typename... args,
          typename... _targs>
...
//
template <typename _Tp, typename _tuple> struct append_tuple_args_first {};
template <template <typename...> class _Template, typename... args,
          typename... _targs>
...
template <typename T> using _type = typename T::type;
template <typename Condition>
using EnableIf = _type<std::enable_if<Condition::value>>;
template <typename Condition>
using DesableIf = _type<std::enable_if<!(Condition::value)>>;
template <typename T> using abstract = typename std::is_abstract<T>::value;
template <typename T, typename method>
using has_member_function =
    std::is_member_function_pointer<decltype(&T::method)>;

template <typename _Tp, typename _tuple>
using append_tuple_args_last_t =
    typename append_tuple_args_last<_Tp, _tuple>::type;
template <typename _Tp, typename _tuple>
using append_tuple_args_first_t =
    typename append_tuple_args_first<_Tp, _tuple>::type;
...
template <typename _Tp, typename X, typename Y> struct replace_X_with_Y {
  using type = _Tp;
};

template <template <typename...> class _Template, typename X, typename Y,
          typename Z, typename... targs>
